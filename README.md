...
- Finance RSS 现支持来源白名单/黑名单和高危阈值告警；满足阈值时会经 ntfy 自动发出一条高危通知。
- Watchlist 行情阈值现在也会走独立的 ntfy 通知，并与 RSS 高危告警分开配置、分开 cooldown。
- `资产统计` 采用“万元人民币”单位，按风险从高到低展示分类（股票、股票基金、黄金、债券/债券基金、货币基金、银行定期存款、银行活期存款、公积金）。
- 支持一键复制资产概况文本（可直接粘贴给大模型咨询资产配置问题）。
- 每次保存都会写入服务端资产快照历史（含时间戳、总资产、各分类金额）；Android 会优先读取服务端历史，并在首次连上服务端时把旧版本遗留的本地历史自动迁移上去。
