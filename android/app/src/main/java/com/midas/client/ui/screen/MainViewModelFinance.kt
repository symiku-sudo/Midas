...
fun MainViewModel.loadHomeOverview() {
    ...
    viewModelScope.launch {
        _homeState.update { it.copy(isLoading = true, errorMessage = "") }
        when (val result = apiRepository.getHomeOverview(baseUrl)) {
            is AppResult.Success -> {
                _homeState.update {
                    it.copy(
                        isLoading = false,
                        generatedAt = result.data.generatedAt,
                        recentTasks = result.data.recentTasks,
                        recentNotes = result.data.recentNotes,
                        financeFocusCards = result.data.financeFocusCards,
                        quickLinks = result.data.quickLinks.map { item ->
                            HomeQuickLinkItemUi(
                                target = item.target,
                                title = item.title,
                        ...
                        assetTotalAmountWan = result.data.assetTotalAmountWan,
                        errorMessage = "",
                    )
                }
            }
